...
export default function BottomNav() {
  ...
  return (
    <div className="fixed right-0 bottom-0 left-0 z-40 flex items-center justify-between bg-white px-12 py-4 text-[12px]">
      ...
      >
        <HomeIcon className={isActive('/home') ? 'fill-primary-500' : 'fill-neutral-300'} />홈
      </button>
      ...
      >
        <StoreIcon className={isActive('/stores') ? 'fill-primary-500' : 'fill-neutral-300'} />
        주변상점
      </button>
      ...
      >
        <PaperIcon className={isActive('/orders') ? 'fill-primary-500' : 'fill-neutral-300'} />
        주문내역
      </button>
    </div>
  );
